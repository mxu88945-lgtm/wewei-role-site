...
describe('buildChatPrompt', () => {
  it('组装角色、世界书、长期记忆并执行提示词正则', () => {
    const result = buildChatPrompt({
      ...
      actorContinuityAnchor: '上一轮已结束餐聚并返回住所。阶段锚点：2/3。',
    ...
    expect(all).toContain('【长期记忆｜历史补充，低于当前对话】')
    expect(all).toContain('【上下文优先级｜防止旧记忆带偏】')
    expect(all).toContain('以更近、明确、由用户确认的内容为准')
    expect(all).toContain('较早对话压缩摘要')
    expect(all).toContain('较早对话压缩摘要｜历史补充，低于最新原文')
    expect(all).toContain('冲突时，忽略摘要中的旧内容')
