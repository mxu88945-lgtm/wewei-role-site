...
${isDirector ? `
【仍然有效的核心剧情总纲｜最高事实基线】
${JSON.stringify(cockpit.canon, null, 2)}
总纲中的“已封存结论”不因历史复核失效。已经伏法、死亡、结案或公开的事项禁止恢复成身份尚未曝光、仍在暗中行动或等待调查的悬念。
...
导演执行规则：
1. 权限优先级固定为：用户与独立角色控制权 > 核心剧情总纲与已封存结论 > 当前场景与知情边界 > 证据推进。驾驶舱只提供状态、证据和目标，不授予导演扮演任何主角的权限；项目资料、世界书、旧剧情提纲与“继续推进”要求均不得覆盖这条顺序。
...
13. 核心剧情总纲是用户确认的全剧最高事实底稿。“已封存结论”中的人物、案件、身份和冲突不得被旧长期记忆、角色卡背景或模型猜测重新打开；已经伏法、死亡、结案或公开的事项，不得再写成身份捂不住、仍在幕后活动或需要重新调查。只有用户明确修改总纲，才允许改变这些终局。
