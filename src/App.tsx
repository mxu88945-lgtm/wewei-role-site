...
function App() {
  ...
  return <div className="app-shell"><main ref={phoneCanvasRef} className={`phone-canvas theme-${chatTheme} ${page === 'chat' ? 'chat-canvas' : ''}`} style={{ '--chat-font-size': `${chatFontSize}px`, '--chat-text-color': chatTextColor, '--chat-narration-color': chatNarrationColor, '--chat-quote-color': chatQuoteColor, '--chat-base-color': chatBaseColor } as React.CSSProperties}>
    ...
    {page === 'more' && <><BackHeader title="设置" onBack={goBack} /><section className="settings-stack compact-settings">{[[['API 连接', 'api'], ['用户身份', 'identity']], [['模型设置', 'model'], ['全局预设', 'preset'], ['全局世界观背景', 'worldbook'], ['长记忆', 'memory']], [['应用设置', 'settings']]].map((group, index) => <div className="settings-group" key={index}>{group.map(([label, target]) => <button key={label} onClick={() => navigate(target as Page)}><span>{label}</span><span>›</span></button>)}</div>)}</section></>}
    ...
    {page === 'worldbook' && <EditablePage title="全局世界观背景" value={worldbook} onChange={setWorldbook} onBack={goBack} fieldLabel="所有窗口共用的世界观" description="这里是惟境的总背景。保存后，所有单聊、群聊以及群聊中的每一位 NPC 都会在生成回复时读到；角色卡自己的世界书仍会另外生效。" note="自动保存于当前设备，并包含在完整备份中。清空内容即可停止全局注入。" placeholder="填写所有角色共同遵守的时代、地域、社会结构、基础规则与已知公共事实……" />}
    ...
    {drawer && <div className="drawer-layer" role="presentation">
      ...
      {drawer === 'right' && <aside className="app-drawer right-drawer" aria-label="聊天设置">
        ...
        <div className="right-drawer-scroll">
          ...
          <section className="drawer-compact-group"><div className="drawer-section-title"><strong>聊天设置</strong></div>{[['情景与角色资料', 'card-data'], ['全局世界观背景', 'worldbook'], ['用户身份', 'identity'], ['主题与背景', 'appearance'], ['字体与文字颜色', 'font']].map(([label, target]) => <button key={label} onClick={() => navigate(target as Page, 'right')}><span>{label}</span><i>›</i></button>)}</section>
