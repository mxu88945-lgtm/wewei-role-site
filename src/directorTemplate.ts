...
${DIRECTOR_OUTPUT_GUARD}`
  ...
  return {
    ...
    beautificationProtocol: '每次回复首个非空字符必须是<scene>...</scene>，随后只输出中文剧情正文，结尾唯一出现<director_status>...</director_status>。场景栏记录时间地点；导演状态栏记录当前外部事件、本轮NPC、公开线索、受限线索、待回应钩子和知情边界。不得在正文输出HTML、CSS、正则或规则说明。',
    ...
    regexScripts: [
      ...
      },
      {
        ...
        scriptName: '共演导演调度状态栏',
        findRegex: '/<director_status>([\\s\\S]*?)<\\/director_status>/gi',
