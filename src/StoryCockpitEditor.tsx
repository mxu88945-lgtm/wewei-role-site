...
export default function StoryCockpitEditor({ project, characters, conversations, userName, api, onBack, onSave, onSetAutoContinuity }: { project: StoryProject; characters: Character[]; conversations: CockpitSourceConversation[]; userName: string; api: ApiConfig; onBack: () => void; onSave: (cockpit: StoryCockpit) => void; onSetAutoContinuity: (enabled: boolean) => void }) {
  ...
  return <>
    ...
    <section className="content-stack story-cockpit-page">
      <div className="cockpit-title-card"><small>当前剧本项目</small><strong>{project.title}</strong><p>{project.autoContinuity.enabled ? '自动场记会在绑定对话完成一轮回复后更新；所有结果仍可在这里检查和修改。' : '手动整理结果先进入草稿，确认保存后才会写入项目。'}</p></div>
      ...
      <section className="cockpit-panel"><header><span>02</span><div><strong>阶段与任务</strong><small>关系不跳级，推进不失焦</small></div></header><label>当前关系阶段<input value={draft.relationshipStage} onChange={(event) => setDraft({ ...draft, relationshipStage: event.target.value })} placeholder="例如：阶段一 · 冷淡与自欺" /></label><label>当前任务<textarea rows={3} value={draft.currentTask} onChange={(event) => setDraft({ ...draft, currentTask: event.target.value })} placeholder="这一幕最需要推进或解决的事情" /></label><label>下一步可推进方向<textarea rows={5} value={text(draft.nextDirections)} onChange={(event) => setDraft({ ...draft, nextDirections: lines(event.target.value) })} placeholder={'每行一个方向\n例如：霍启铭带来新的合作条件'} /></label></section>
